...
angular.module('playtogether').controller('SpeedRiskCtl', ['$scope', '$document', '$http',
	function ($scope, $document, $http) {
		...
		var players, country, countries, stage, renderer, public_data, private_data, themes, state, id, backgroundImg, assetsLoaded;
		...
		$scope.$on('armies', function (event, data) {
			private_data.armies = data.armies;
		});
		...
		function animate() {
			if (assetsLoaded) {
			for (var i = 0; i < public_data.countries.length; i++) {
		...
		function overlay(c_id, image, loc) {
			...
			sprite.click = sprite.tap = function (e) {
                console.log(public_data.countries[c_id], id);
				if (e.originalEvent.ctrlKey === true && public_data.countries[c_id].owner == id) {
                    console.log(private_data);
					$scope.ws.send({cmd: 'place', country: c_id, armies: private_data.armies});
				}
				else {
