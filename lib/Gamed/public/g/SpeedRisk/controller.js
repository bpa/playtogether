define(['js/pixi'], function(PIXI) {
	function SpeedRiskCtl($scope, $document, $http, $modal) {
		...
		function overlay(c_id, image, loc) {
			...
			sprite.click = sprite.tap = function (e) {
				...
				if (owned && (e.originalEvent.ctrlKey === true || e.originalEvent.metaKey === true)) {
...
//							s.panel.setTexture(PIXI.Texture.fromFrame(players[p].theme + '_background'));
    ...
    SpeedRiskCtl.$inject = ['$scope', '$document', '$http', '$modal'];
    return SpeedRiskCtl;
});
