...
angular.module('playtogether').controller('SpeedRiskCtl', ['$scope', '$document', '$http', '$modal',
	function ($scope, $document, $http, $modal) {
		...
		var players, country, countries, stage, renderer, themes, state, id, backgroundImg, assetsLoaded, hud, pub;
		...
		function overlay(c_id, image, loc) {
			...
			sprite.click = sprite.tap = function (e) {
				if (e.originalEvent.ctrlKey === true && pub.countries[c_id].owner == id) {
		...
		Hud.prototype.make_player_row = function(p) {
			...
			e.readyText = new PIXI.Text(e.ready ? "(/)" : "(X)", {font: "15px Ariel"});
			e.readyText.x = 5;
			e.readyText.y = 5;
			e.panel.addChild(e.readyText);
			var nameText = new PIXI.Text(e.name, {font: "15px Ariel"});
			nameText.x = 30;
			nameText.y = 5;
