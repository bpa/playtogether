var RoboRally = function (socket) {
  var stage, viewport, renderer, players, public, id, private, now, cards, board, modal, ready_button;
  var direction_in_radians = [0, Math.PI/2, Math.PI, Math.PI * 1.5];
  var assetsLoaded = false;
  var pieces = {}, cards = {}, textures = {};
  var tween = [];
  var step = 750;
  var through = 0;

  function conveyor(belt) {
    var texture = new PIXI.RenderTexture(64, 64, renderer, 0, renderer.resolution);
    texture.rr_disp = new PIXI.DisplayObjectContainer();
    texture.rr_base = PIXI.Sprite.fromFrame('conveyor_base');
    texture.rr_belt = PIXI.Sprite.fromFrame(belt);
    texture.rr_disp.addChild(texture.rr_base);
    texture.rr_disp.addChild(texture.rr_belt);
    texture.render(texture.rr_disp);
    return texture;
  }
  ...
  function onAssetsLoaded() {
    ...
    viewport.addChild(button("Test", public.course.width * 64 + 96, public.course.height * 64, function(data){
          tween.push(animate_conveyor(textures.conveyor, step));
        }));
  ...
  function create_board() {
    ...
    for (var x=0; x<public.course.width; x++) {
      ...
      for (var y=0; y<public.course.height; y++) {
        ...
        if (tile.t !== undefined) {
          var texture = textures[tile.t] === undefined ? PIXI.Texture.fromFrame(tile.t) : textures[tile.t];
          ...
          if ($.inArray(tile.t, ['pit', 'upgrade', 'wrench']) != -1)
    ...
    viewport.addChildAt(new PIXI.Sprite(boardTexture), 0);
  }

  function initialize_textures() {
    textures.conveyor = conveyor('conveyor_belt');
    textures.conveyor2 = conveyor('conveyor2_belt');
  }

  function wall(x, y, rotation) {
    var w = PIXI.Sprite.fromFrame("wall");
    w.anchor.x = .5;
    w.anchor.y = .5;
    w.x = x;
    w.y = y;
  ...
  function create_walls() {
    var x = Math.pow(2, Math.ceil(Math.log(public.course.width * 64) / Math.log(2)));
    var y = Math.pow(2, Math.ceil(Math.log(public.course.height * 64) / Math.log(2)));
    var texture = new PIXI.RenderTexture(x, y, renderer, 0, renderer.resolution);
    texture.crop = new PIXI.Rectangle(0, 0, public.course.width * 64, public.course.height * 64);
    var walls = new PIXI.DisplayObjectContainer();
    for (var x=0; x<public.course.width; x++) {
      var c_x = x * 64;
      for (var y=0; y<public.course.height; y++) {
        var c_y = y * 64;
        var w = public.course.tiles[y][x].w;
        if (w) {
          if (w & 1) walls.addChild(wall(c_x + 32, c_y + 4, 0));
          if (w & 2) walls.addChild(wall(c_x + 60, c_y + 32, Math.PI / 2));
          if (w & 4) walls.addChild(wall(c_x + 32, c_y + 60, 0));
          if (w & 8) walls.addChild(wall(c_x + 4, c_y + 32, Math.PI / 2));
        }
      }
    }
    texture.render(walls);
  ...
  function rotate(sprite, dir, time, start) {
  ...
  function move(sprite, x, y, time, start) {
    if (start === undefined) start = now;
    var end = start + time;
    var orig_x, orig_y;
  ...
  function moveTo(sprite, x, y, time, start) {
    if (start === undefined) start = now;
    var end = start + time;
    var orig_x = sprite.x;
    var orig_y = sprite.y;
  ...
  function fall(sprite, time, start) {
    if (start === undefined) start = now;
    var end = start + time;
  ...
  function animate_conveyor(conveyor, time, start) {
    if (start === undefined) start = now;
    var end = start + time;
    ...
    return function() {
      ...
      if (now >= end) {
        conveyor.rr_belt.y = 0;
        conveyor.render(conveyor.rr_disp);
        return false;
      }

      var remaining = (end - now) / time;
      conveyor.rr_belt.y = -64 + 64 * remaining;
      conveyor.render(conveyor.rr_disp);
      return true;
    }
  }
  ...
  this.on_execute = function(ev, data) {
    var watermark = now > through ? now : through;
    if (data.phase === 'conveyors') {
      tween.push(animate_conveyor(textures.conveyor, step, through));
      tween.push(animate_conveyor(textures.conveyor2, step, through));
    }
    if (data.phase === 'express_conveyors') {
      tween.push(animate_conveyor(textures.conveyor2, step, through));
    }
    for (var i=0; i<data.actions.length; i++) {
      ...
      var duration = 0;
      if (action.length)
        duration = action[0].move === undefined ? .5 : action[0].move;
      watermark += step * duration;
      for (var j=0; j<action.length; j++) {
        ...
        if (a.move !== undefined) {
          ...
          console.log(step, a, watermark);
          tween.push(move(sprite, x, y, step * a.move, watermark - step * a.move));
          if (a.die === 'fall') tween.push(fall(sprite, step, watermark - (step * (a.move - 1))));
        }
        if (a.rotate !== undefined) {
          if (a.move === undefined) {
            tween.push(rotate(sprite, a.rotate, step, watermark - step));
          }
          else {
            tween.push(rotate(sprite, a.rotate, step / 2, watermark - step / 2));
          }
        }
      }
    }
    through = Math.max(through + step, watermark);
  }
