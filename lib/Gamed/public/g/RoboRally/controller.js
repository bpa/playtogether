var RoboRally = function (socket) {
  var stage, renderer, players, public, id, private, now, cards, board, modal, ready_button;
  ...
  var on_program = this.on_program = function(ev, data) {
    ...
    for (var i=0; i<data.registers.length; i++) {
      ...
      for (var j=0; j<register.length; j++) {
        ...
        card.rr_locked = true;
        card.rr_register = i;
        tween.push(move(cards[register[j]], i * 128, public.course.height * 64, 500));
      }
    }
    for (var c in cards) {
      if (!(c in used)) {
        ...
        tween.push(move(card, card.rr_x, card.rr_y, 500));
      }
    }
  }
    ...
    for (var i=0; i<data.actions.length; i++) {
      ...
      for (var j=0; j<action.length; j++) {
        ...
        if (a.move !== undefined) {
          ...
          tween.push(move(sprite, sprite.x + x, sprite.y + y, 1000 * a.move, 1000 * delay));
          delay += a.move;
        }
        if (a.rotate !== undefined) {
          if (a.move === undefined) {
            tween.push(rotate(sprite, a.rotate, 1000, 1000 * delay));
            delay++;
          }
          else {
            tween.push(rotate(sprite, a.rotate, 500, 1000 * delay - 500));
          }
        }
      }
    }
