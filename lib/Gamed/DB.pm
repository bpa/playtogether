...
unless ( $dbh->selectrow_arrayref("SELECT 1 FROM sqlite_master WHERE type='table' AND name = 'user'") ) {
    $dbh->do("create table users (username text PRIMARY KEY, passphrase, name text, avatar text)");
}

helper login => sub {
    my $self = shift;
    my $user = $dbh->selectall_arrayref( "select * from users where username=?", { Slice => {} }, $self->param('username'), );
