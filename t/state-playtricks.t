...
$s->game( { cmd => 'play', card => 8 }, { cmd => 'error', reason => 'Invalid card', card => 8 } );
...
broadcasted( $game, $n, { cmd => 'play', card => 1 }, { player => 0, card => 1 }, 'N plays a 1' );
is_deeply( $game->{public}{trick}, [ 4, 1 ], 'Card added to trick' );
$e->broadcast( { cmd => 'play', card => 2 }, { trick => [ 4, 1, 2 ], winner => 2 }, 'Trick winner declared' );
...
broadcasted( $game, $s, { cmd => 'play', card => 8 }, { player => 2, card => 8 }, 'Round 2' );
broadcasted( $game, $n, { cmd => 'play', card => 5 }, { player => 0, card => 5 }, 'Round 2' );
$e->broadcast( { cmd => 'play', card => 9 }, { trick => [ 8, 5, 9 ], winner => 1 }, 'Trick winner declared' );
is_deeply( $game->{players}{1}{taken}, [ 8, 5, 9 ], "Captured trick given to player" );

broadcasted( $game, $e, { cmd => 'play', card => 11 }, { player => 1, card => 11 }, 'Round 3' );
broadcasted( $game, $s, { cmd => 'play', card => 12 }, { player => 2, card => 12 }, 'Round 3' );
$n->broadcast( { cmd => 'play', card => 13 }, { trick => [ 11, 12, 13 ], winner => 0 }, 'Trick winner declared' );
is_deeply( $game->{players}{0}{taken}, [ 11, 12, 13 ], "Captured trick given to player" );

broadcasted( $game, $n, { cmd => 'play', card => 14 }, { player => 0, card => 14 }, 'Round 3' );
broadcasted( $game, $e, { cmd => 'play', card => 15 }, { player => 1, card => 15 }, 'Round 3' );
$s->broadcast( { cmd => 'play', card => 16 }, { trick => [ 14, 15, 16 ], winner => 2 }, 'Trick winner declared' );
is_deeply(
